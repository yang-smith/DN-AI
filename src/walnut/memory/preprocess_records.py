...
from walnut.memory.prompt.preprocess import prompt_split, prompt_analyze, prompt_analyze_dig, prompt_analyze_summary
...
def write_daily_record_to_file(daily_texts, date, output_dir='./output/daily_records'):
    os.makedirs(output_dir, exist_ok=True)

    output_file = os.path.join(output_dir, f"record_{date.strftime('%Y_%m_%d')}.txt")

    with open(output_file, 'w', encoding='utf-8') as f:
        if date in daily_texts:
            f.write(daily_texts[date])
            print(f"Daily record for {date} has been written to {output_file}")
        else:
            f.write(f"No records available for {date}")
            print(f"No records found for {date}. Empty file created at {output_file}")


prompt_tem = """
请从以下聊天记录中提取美食相关的信息，并按照以下结构化格式输出：
...
注意事项：
1. 只提取聊天记录中明确提到的正式餐厅信息，不要包括食堂、九里柴火房、厨房或家庭烹饪。
2. 不要列出食材、特产或非餐厅相关的信息。
3. 如果某项信息未提及，请省略该项。
4. 如果记录中包含多个餐厅，请分别列出。
5. 如果没有相关信息，请直接回答"没有"
"""

def analyze_food_info(daily_texts, year, month, output_dir='./food_analysis'):
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, f"food_analysis_{year}_{month:02d}.txt")
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        for day in range(1, days_in_month + 1):
            ...
            records = daily_texts.get(current_date)

            f.write(f"\n--- Date: {current_date} ---\n")
            if records:
                prompt = prompt_tem.format(records=records)
                result = ai_chat(prompt, 'gpt-4o-mini')
                f.write(result + "\n")
            else:
                f.write("No records for this date.\n")

    print(f"Food analysis for {year}-{month:02d} completed. Results saved to {output_file}")
...
async def main():
    ...
    analyze_food_info(daily_texts, year, 9)
    analyze_food_info(daily_texts, year, 7)
    analyze_food_info(daily_texts, year, 8)
    # prompt = prompt_split(records)
    # # print(prompt)
    # result = ai_chat(prompt, 'gpt-4o-mini')
    # print(result)
