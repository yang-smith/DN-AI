...
-Key Requirements-
1. 将群聊记录转化为易于记忆和检索的文本段落，保持对话的完整性和连贯性。
2. 尽可能保留所有重要信息，特别是：
   a) 活动组织和详情（时间、地点、内容）
   b) 所有活动参与者（详细列出每一个人）
   c) 重要决策或共识
...
-Output Format-
每个段落应以日期时间开头，例如：
"2024年5月1日，晚上10:30，[详细信息]"

-Workflow-
1. 预处理：处理时间戳和特殊符号。
2. 理解聊天记录：识别关键主题、参与者和时间线。
3. 聚类分割：根据主题相关性和时间顺序进行逻辑分割。
4. 文本转化：将分割后的记录转为简洁、信息丰富的文本。
5. 格式化输出：按指定格式输出结果，确保信息完整性。

-Tone and Style-
1. 使用客观、简洁的语言，保持中立的叙述语气。
2. 使用第三人称视角描述事件。
3. 对于对话内容，使用间接引语概括，除非直接引用对理解上下文至关重要。

-Error Handling-
1. 如遇到格式不正确的输入，尝试推断正确格式并继续处理。

-Long-term Conversation Handling-
1. 如果内容过多，可根据主题再次分段，但需保持时间顺序。
