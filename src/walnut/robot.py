...
import walnut.memory
import walnut.agents.baishi as baishi
import walnut.agents.customer_services as customer_services
import walnut.agents.judger as judger
...
class Robot():
    ...
    async def processMsg(self, msg: WxMsg) -> None:
        ...
        if msg.from_group():
            ...
            #     return
            if msg.content.startswith('核桃，切换成小客服模式') or msg.content.startswith('核桃，切换小客服模式'):
                self.type = 'customer_services'
                self.sendTextMsg('小客服模式已开启', msg.roomid, msg.sender)
                return
            if msg.content.startswith('核桃，切换成核桃模式') or msg.content.startswith('核桃，切换核桃模式'):
                self.type = 'baishi'
            ...
            if msg.is_at(self.wxid) or msg.content.startswith('核桃') or self.state[msg.roomid] == 'running':
                if self.type == 'baishi':
                    rsp, check = await asyncio.gather(
                        self.baishi_agent.query(msg.roomid, msg.content, model='gpt-4o-2024-08-06'),
                        self.judger_agent.check(msg.roomid, msg.sender+'：'+msg.content, model='gpt-4o-mini')
                    )
                elif self.type == 'customer_services':
                    rsp, check = await asyncio.gather(
                        self.customer_services_agent.query(msg.roomid, msg.content, model='gpt-4o-2024-08-06'),
                        self.judger_agent.check(msg.roomid, msg.sender+'：'+msg.content, model='gpt-4o-mini')
                    )
                ...
                if rsp and check:
                    ...
                    self.sendTextMsg('没我事情，我就下线了。还有事情，招呼核桃。', msg.roomid, msg.sender)
                    self.state[msg.roomid] = 'waiting'
